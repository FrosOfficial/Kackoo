 ...
 */
export async function loadTermSettings(): Promise<TermSettings> {
  try {
    const termType =
      ((await AsyncStorage.getItem(TERM_TYPE_KEY)) as TermType) || "trimester";
    ...
    const learningMode =
      ((await AsyncStorage.getItem(LEARNING_MODE_KEY)) as LearningMode) ||
      "blended";
    ...
    const inpersonOffset = inpersonOffsetStr
      ? parseInt(inpersonOffsetStr, 10)
      : 15;
    const onlineWeekPattern =
      ((await AsyncStorage.getItem(
        ONLINE_WEEK_PATTERN_KEY
      )) as OnlineWeekPattern) || "even";
    const alarmSound =
      ((await AsyncStorage.getItem(ALARM_SOUND_KEY)) as AlarmSound) ||
      "default";
    return {
      termType,
      totalWeeks,
 ...
 */
export async function setAppInitialized(val: boolean): Promise<void> {
  try {
